...
# 告知
tellraw @a[tag=Auction.Participant] [{"text":"[","color": "white"},{"text":"Auction","color": "dark_purple"},{"text":"] ","color": "white"},{"text":"1位の入札をキャンセルし、2位の入札を繰り上げ1位にしました。","color": "red","bold": false}]
tellraw @a[tag=Auction.Participant] [{"selector":"@a[tag=Auction.TopBidder]"},{"text":"さんが"},{"score":{"name": "$BidTop","objective": "AuctionBid"},"color": "gold","bold": true},{"text": "MP","color": "gold","bold": true},{"text": "で1位になりました！"}]
